...
function OperacaoGaragemSubstituirPatrimonioScreen(props) {
    ...
    async function confirmar_substituicao(){
        ...
        await OperacaoGaragemService.substituir_patrimonio({
    ...
    return (
        ...
        <View style={style.container}>
            ...
            <ProgressSteps removeBtnRow={true} activeStep={active}  completedStepIconColor="orange" activeStepIconBorderColor="orange" completedProgressBarColor="orange" labelColor="black" activeLabelColor="black">
                ...
                <ProgressStep style={{display: 'none'}} removeBtnRow={true} nextBtnDisabled={true} nextBtnTextStyle={{display: 'none'}} label="Entrada"  previousBtnStyle={{display: 'none'}}>
                    <OperacaoGaragemAssociarPatrimonio substituir={true} setEquipamentoEntrada={setEquipamentoEntrada} setActive={setActive} />
                </ProgressStep>
                <ProgressStep removeBtnRow={true} nextBtnDisabled={true} nextBtnTextStyle={{display: 'none'}} label="Confirmar"  previousBtnStyle={{display: 'none'}}>
                    {
                        active === 3 ? (
                            <View style={{ marginHorizontal: 10, marginBottom: 10 }}>
                                <LottieQuestion titulo="Confirma a substituição?"></LottieQuestion>
                                ...
                                <View style={{marginVertical: 10}}>
                                    <Text style={{fontSize: 20, fontWeight: 'bold', borderColor: 'lightgray', borderBottomWidth: 1, marginBottom: 5}}>Associando:</Text>
                                    <View style={{flexDirection: 'row', marginVertical: 2}}>
                                        <Text style={{fontWeight: 'bold', fontSize: 18}}>Patrimônio: </Text>
                                        <Text style={{fontSize: 18}}>{equipamento_entrada.patrimonio}</Text>
                                    </View>
                                </View>
                            </View>
                        ) : (<></>)
        ...
        <View style={style.container_button}>
            ...
            {
                active == 3 && equipamento_entrada !== null ? (<AppButton onPress={() => { confirmar_substituicao() }}>Confirmar</AppButton>) : (<></>)
            }
        </View>
        </>
    )
...
export default OperacaoGaragemSubstituirPatrimonioScreen;